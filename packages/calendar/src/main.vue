...
import fecha from 'cmstop-ui/src/utils/date';
import ElButton from 'cmstop-ui/packages/button';
import ElButtonGroup from 'cmstop-ui/packages/button-group';
import DateTable from './date-table';
import { validateRangeInOneMonth } from 'cmstop-ui/src/utils/date-util';
  ...
  methods: {
    ...
    rangeValidator(date, isStart) {
      ...
      if (date.getDay() !== expected) {
        console.warn('[CmstopCalendar]', message, 'Invalid range will be ignored.');
        return false;
      }
      return true;
    }
  ...
  computed: {
    ...
    validatedRange() {
      ...
      if (range.length === 2) {
        ...
        if (start > end) {
          console.warn('[CmstopCalendar]end time should be greater than start time');
          return [];
        }
        ...
        if (!validateRangeInOneMonth(startDay, end)) {
          console.warn('[CmstopCalendar]start time and end time interval must not exceed two months');
          return [];
        }
        // 第一个月的时间范围
        data.push([
